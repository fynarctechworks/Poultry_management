...
// and housing.
//
// WHY THIS LIVES IN @poultryos/shared (not a DB table):
//   - Static reference data, rarely changes; no per-tenant customisation yet.
//   - The fuzzy breed-name match + tone bands are non-trivial logic that both
//     the Expo app and the Next.js dashboard need from one canonical source.
//   - Computing insights client-side from this constant keeps dashboards fast
//     and the <60s daily-log path untouched (no RPC round-trips).
//   When owners can edit benchmarks, or a server-side WhatsApp digest needs
//   them, seed a DB table from these same values then — not before.
//
...
export interface BreedBenchmark {
  ...
  cycleDays: number;
  /** Layer only: peak hen-day egg production % (~weeks 28–32). null otherwise */
  peakHdepPct?: number | null;
  /** Layer only: typical feed intake per bird per day at peak (grams) */
  feedPerBirdPerDayG?: number | null;
}
