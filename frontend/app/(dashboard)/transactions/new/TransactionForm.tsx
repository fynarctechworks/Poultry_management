...
export function TransactionForm({ farms, buyers, batches }: Props) {
  ...
  async function onSubmit(data: Form) {
    ...
    const payload: any = {
      ...
      // amount_paid only refines a partial; 'paid'/'pending' derive outstanding
      // from status server-side, so leave it null for them.
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="card space-y-md">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
        ...
        <Field label="Category" error={errors.category?.message}>
          <input className="input" placeholder={txType === 'income' ? 'broiler_sale / egg_sale' : 'feed / medicine / labour'} {...register('category')} />
        </Field>
